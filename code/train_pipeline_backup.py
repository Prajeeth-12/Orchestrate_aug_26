...
try:
    ...
    GPU_AVAILABLE = False
    print("⚠ PyTorch not available - GPU monitoring disabled")
...
class MessageRoutingPipeline:
    ...
    def train(
        ...
        """
        print("\n" + "="*80)
        print("🚀 TRAINING MESSAGE ROUTING PIPELINE (GPU-ACCELERATED)")
        print("="*80)
        ...
        if use_gpu and GPU_AVAILABLE:
            ...
            print(f"\n🎮 GPU acceleration enabled (Device {gpu_id})")

            if torch.cuda.is_available():
                print(f"📊 Initial GPU Memory: {torch.cuda.memory_allocated(gpu_id)/1024**2:.0f} MB / {torch.cuda.get_device_properties(gpu_id).total_memory/1024**2:.0f} MB")
        else:
        ...
        # Train model
        print(f"\n🎯 Training XGBoost ({n_estimators} estimators, max_depth={max_depth}, lr={learning_rate})...")
        print("-" * 80)
        ...
        if use_gpu and GPU_AVAILABLE and torch.cuda.is_available():
            print(f"\n📊 Final GPU Memory: {torch.cuda.memory_allocated(gpu_id)/1024**2:.0f} MB / {torch.cuda.get_device_properties(gpu_id).total_memory/1024**2:.0f} MB")
